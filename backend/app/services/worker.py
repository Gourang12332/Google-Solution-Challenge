...
def check_authorization(authority_id: str, platform: str, uploader: str) -> str:
    wl = whitelist_col.find_one({"authority_id": authority_id, "platform": platform}, {"_id": 0})
    if not wl:
        return "unauthorized"
...
def process_one_pending() -> None:
    ...
    if not pending:
        return
    try:
        local_video_path = download_video(pending["video_url"], INCOMING_DIR)
        detection_type = None
        media_doc = None
        match_score = 0.0
        ...
        if watermark_id:
            media_doc = official_media_col.find_one({"watermark_id": watermark_id}, {"_id": 0})
            if media_doc:
                detection_type = "watermark"
                match_score = 1.0

        if not media_doc:
            frames = extract_frame_hashes(local_video_path, max_frames=20, sample_step=30)
            media_doc, match_score = find_best_official_match(frames, ratio_threshold=0.4)
            if media_doc:
                detection_type = "fingerprint"

        if media_doc and detection_type:
            uploader = uploader_from_snippet(pending)
            status = check_authorization(media_doc["authority_id"], pending["platform"], uploader)
            detections_col.insert_one(
                {
                    "detection_id": uuid.uuid4().hex,
                    "media_id": media_doc["media_id"],
                    "source": {"platform": pending["platform"], "uploader": uploader},
        ...
        incoming_media_col.update_one(
            {"video_id": pending["video_id"]},
            {"$set": {"status": status}, "$inc": {"retry_count": 1}},
        )
